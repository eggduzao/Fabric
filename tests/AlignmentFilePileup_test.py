...
class TestPileupQueryPosition(unittest.TestCase):
    ...
    def testPileup(self):
        last = {}
        with fabric.AlignmentFile(os.path.join(BAM_DATADIR, self.filename)) as inf:
...
class TestIteratorColumnBAM(unittest.TestCase):
    ...
    def setUp(self):
        self.samfile = fabric.AlignmentFile(os.path.join(BAM_DATADIR, "ex4.bam"),
                                           "rb")
...
class PileUpColumnTests(unittest.TestCase):
    ...
    def test_pileup_depths_are_equal(self):
        ...
        fabric_result = PileupTestUtils.build_depth_with_filter_with_fabric(self.fn)
        self.assertEqual(fabric_result, samtools_result)
    ...
    def test_pileup_query_bases_without_reference_are_equal(self):
        ...
        fabric_result = PileupTestUtils.build_query_bases_with_fabric(self.fn)
        self.assertEqual(["".join(x) for x in fabric_result], samtools_result)
    ...
    def test_pileup_query_bases_with_reference_are_equal(self):
        ...
        with fabric.FastaFile(self.fn_fasta) as fasta:
            fabric_result = PileupTestUtils.build_query_bases_with_fabric(self.fn, fastafile=fasta, stepper="samtools")
        self.assertEqual(["".join(x) for x in fabric_result], samtools_result)
    ...
    def test_pileup_query_qualities_are_equal(self):
        ...
        fabric_result = PileupTestUtils.build_query_qualities_with_fabric(self.fn)
        fabric_result = [
            [chr(min(126, x + 33)) for x in l] for l in fabric_result]
        self.assertEqual("".join(flatten_nested_list(fabric_result)),
    ...
    def test_pileup_mapping_qualities_are_equal(self):
        ...
        fabric_result = PileupTestUtils.build_mapping_qualities_with_fabric(self.fn)
        # convert to chars
        fabric_result = [
            [chr(min(126, x + 33)) for x in l] for l in fabric_result]
    ...
    def test_pileup_query_qualities_from_pileups_are_equal(self):
        samtools_result = PileupTestUtils.build_query_qualities_with_samtoolspipe(self.fn)
        fabric_result = PileupTestUtils.build_query_qualities_with_fabric_pileups(self.fn)
        fabric_result = [
            "".join([chr(min(126, x + 33)) for x in l]) for l in fabric_result]
