...
def build_fetch_with_fabric(*args, **kwargs):
...
def build_query_sequences_with_fabric(*args, **kwargs):
    with fabric.AlignmentFile(*args, **kwargs) as inf:
...
def build_query_sequences_flagfilter_with_fabric(*args, **kwargs):
    with fabric.AlignmentFile(*args, **kwargs) as inf:
        data = [x.query_sequence for x in inf if x.is_proper_pair]
    return data


def build_query_sequences_directflagfilter_with_fabric(*args, **kwargs):
    with fabric.AlignmentFile(*args, **kwargs) as inf:
        data = [x.query_sequence for x in inf if x.flag & 2]
    return data


def build_aligned_pairs_with_fabric(*args, **kwargs):
