...
class TestOpening(unittest.TestCase):
    ...
    def testMissingFile(self):
        self.assertRaises(IOError, fabric.VariantFile,
                          "missing_file.vcf")
    ...
    def testEmptyFileVCF(self):
        with get_temp_context("tmp_testEmptyFile.vcf") as fn:
            with open(fn, "w"):
                pass
            self.assertRaises(ValueError, fabric.VariantFile, fn)

    def testEmptyFileVCFFromPath(self):
        with get_temp_context("tmp_testEmptyFile.vcf") as fn:
            with open(fn, "w"):
                pass
            self.assertRaises(ValueError, fabric.VariantFile, Path(fn))
    ...
    def testEmptyFileVCFGZWithIndex(self):
        with get_temp_context("tmp_testEmptyFile.vcf") as fn:
            ...
            fabric.tabix_index(fn,
                              preset="vcf",
                              force=True)

            self.assertRaises(ValueError, fabric.VariantFile, fn + ".gz")
    ...
    def testEmptyFileVCFOnlyHeader(self):
        with fabric.VariantFile(os.path.join(
                CBCF_DATADIR,
    ...
    def testDetectVCF(self):
        with fabric.VariantFile(os.path.join(CBCF_DATADIR,
...
class TestIndexFormatsVCF(unittest.TestCase):
    ...
    def test_vcf_with_tbi_index(self):
        with get_temp_context("tmp_fn.vcf") as fn:
            ...
            fabric.tabix_index(fn, preset="vcf", force=True)
            ...
            with fabric.VariantFile(fn + ".gz") as inf:
    ...
    def test_vcf_with_csi_index(self):
        with get_temp_context("tmp_fn.vcf") as fn:
            ...
            fabric.tabix_index(fn, preset="vcf", force=True, csi=True)
    ...
    def test_bcf_with_tbi_index_will_produce_csi(self):
        with get_temp_context("tmp_fn.bcf") as fn:
            ...
            fabric.tabix_index(fn, preset="bcf", force=True, csi=False)
            self.assertTrue(os.path.exists(fn + ".csi"))
            self.assertEqual(read_index_header(fn + ".csi"), b"CSI\1")
            self.assertFalse(os.path.exists(fn + ".tbi"))
...
# is because htslib's parser is lazy and the fabric API needs to trigger
...
class TestIndexFilename(unittest.TestCase):
    ...
    def testOpen(self):
        for fn, idx_fn in self.filenames:
            ...
            with fabric.VariantFile(fn, index_filename=idx_fn) as inf:
                self.assertEqual(len(list(inf.fetch('20'))), 3)
...
class TestConstructionVCFWithContigs(unittest.TestCase):
    ...
    def testBase(self):
        with fabric.VariantFile(os.path.join(CBCF_DATADIR, self.filename)) as inf:
            self.assertEqual(inf.category, 'VARIANTS')
            self.assertEqual(inf.format, 'VCF')
    ...
    def testConstructionWithRecords(self):
        ...
        vcf_in = fabric.VariantFile(fn_in)

        header = fabric.VariantHeader()
        ...
        vcf_out = fabric.VariantFile(fn_out, "w", header=header)
    ...
    def testConstructionFromCopy(self):
        ...
        vcf_out = fabric.VariantFile(fn_out, "w", header=vcf_in.header)
        for record in vcf_in:
            vcf_out.write(record)
...
class TestMultiThreading(unittest.TestCase):
    ...
    def testSingleThreadEqualsMultithreadResult(self):
        with fabric.VariantFile(self.filename) as inf:
            header = inf.header
            single = [r for r in inf]
        with fabric.VariantFile(self.filename, threads=2) as inf:
            multi = [r for r in inf]
        for r1, r2 in zip(single, multi):
            assert str(r1) == str(r2)

        bcf_out = get_temp_filename(suffix=".bcf")
        with fabric.VariantFile(bcf_out, mode='wb',
                               header=header,
                               threads=2) as out:
            for r in single:
                out.write(r)
        with fabric.VariantFile(bcf_out) as inf:
    ...
    def testNoMultiThreadingWithIgnoreTruncation(self):
        with self.assertRaises(ValueError):
            fabric.VariantFile(self.filename,
                              threads=2,
