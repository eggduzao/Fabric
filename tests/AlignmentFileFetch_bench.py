...
def test_build_fetch_from_bam_with_fabric(benchmark):
    result = benchmark(build_fetch_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert result == 3270
...
def test_build_query_sequences_from_bam_with_fabric(benchmark):
    result = benchmark(build_query_sequences_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert len(result) == 3270
...
def test_build_query_sequences_from_bam_flagfilter_with_fabric(benchmark):
    result = benchmark(build_query_sequences_flagfilter_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert len(result) == 3124


def test_build_query_sequences_from_bam_directflagfilter_with_fabric(benchmark):
    result = benchmark(build_query_sequences_flagfilter_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert len(result) == 3124
...
def test_build_aligned_pairs_default_with_fabric(benchmark):
    result = benchmark(build_aligned_pairs_with_fabric,
                       os.path.join(BAM_DATADIR, "with_md.bam"))
    assert len(result) == 3235


@pytest.mark.aligned_pairs
def test_build_aligned_pairs_matchesonly_with_fabric(benchmark):
    result = benchmark(build_aligned_pairs_with_fabric,
                       os.path.join(BAM_DATADIR, "with_md.bam"),
                       matches_only=True)
    assert len(result) == 3235
...
@pytest.mark.aligned_pairs    
def test_build_aligned_pairs_withseq_with_fabric(benchmark):
    result = benchmark(build_aligned_pairs_with_fabric,
                       os.path.join(BAM_DATADIR, "with_md.bam"),
                       with_seq=True)
    assert len(result) == 3235
