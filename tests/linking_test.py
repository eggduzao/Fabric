...
@unittest.skipUnless(
    os.environ.get("FABRIC_LINKING_TESTS", None),
    "enable linking tests by setting FABRIC_LINKING_TESTS environment variable")
...
class TestLinkWithoutRpath(TestLinking):
    ...
    def test_package_tests_pass_if_ld_library_path_set(self):
        ...
        fabric_libdirs, fabric_libs = zip(
            *[os.path.split(x) for x in fabric_libraries])
