...
def test_build_pileup_from_bam_with_fabric(benchmark):
    result = benchmark(build_pileup_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert result == 2998
...
def test_build_depth_from_bam_with_fabric(benchmark):
    result = benchmark(build_depth_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
...
def test_build_depth_with_filter_from_bam_with_fabric(benchmark):
    result = benchmark(build_depth_with_filter_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert sum(result) == 107241
...
def test_build_query_bases_from_bam_with_samtoolsfabric(benchmark):
    result = benchmark(build_query_bases_with_samtoolsfabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
...
def test_build_query_bases_from_bam_with_fabric_pileups(benchmark):
    # note that there is no overlap detection here
    result = benchmark(build_query_bases_with_fabric_pileups,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert len("".join(flatten_nested_list(result))) == 107241


def test_build_query_bases_from_bam_with_fabric(benchmark):
    result = benchmark(build_query_bases_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert len("".join(flatten_nested_list(result))) == 116308
...
def test_build_query_bases_with_reference_from_bam_with_fabric(benchmark):
    with fabric.FastaFile(os.path.join(BAM_DATADIR, "ex1.fa")) as fasta:
        result = benchmark(build_query_bases_with_fabric,
                           os.path.join(BAM_DATADIR, "ex2.bam"),
                           fastafile=fasta)
    assert len("".join(flatten_nested_list(result))) == 115924
...
def test_build_query_bases_with_reference_from_bam_with_samtoolsfabric(benchmark):
    result = benchmark(build_query_bases_with_samtoolsfabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"),
                       "-f", os.path.join(BAM_DATADIR, "ex1.fa"))
    assert len("".join(flatten_nested_list(result))) == 115924
...
def test_build_query_qualities_from_bam_with_fabric(benchmark):
    result = benchmark(build_query_qualities_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert sum([len(x) for x in result]) == 107241


def test_build_query_names_from_bam_with_fabric(benchmark):
    result = benchmark(build_query_names_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
...
def test_build_mapping_qualities_from_bam_with_fabric(benchmark):
    result = benchmark(build_mapping_qualities_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert sum([len(x) for x in result]) == 107241
...
def test_build_query_positions_from_bam_with_fabric(benchmark):
    result = benchmark(build_query_positions_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
