...
class TestIndexing(unittest.TestCase):
    ...
    def test_indexing_with_preset_works(self):
        ...
        fabric.tabix_index(self.tmpfilename, preset="gff")
    ...
    def test_indexing_with_explict_columns_works(self):
        ...
        fabric.tabix_index(self.tmpfilename,
                          seq_col=0,
                          start_col=3,
                          end_col=4,
                          line_skip=0,
...
class TestCompression(unittest.TestCase):
    ...
    def testCompression(self):
        '''see also issue 106'''
        fabric.tabix_compress(self.tmpfilename, self.tmpfilename + ".gz")
        checkBinaryEqual(self.tmpfilename, self.tmpfilename + ".gz")

    def testIndexPresetUncompressed(self):
        '''test indexing via preset.'''

        fabric.tabix_index(self.tmpfilename, preset=self.preset)
    ...
    def testIndexPresetCompressed(self):
        ...
        fabric.tabix_compress(self.tmpfilename, self.tmpfilename + ".gz")
        fabric.tabix_index(self.tmpfilename + ".gz", preset=self.preset)
        checkBinaryEqual(self.tmpfilename + ".gz", self.filename)
        checkBinaryEqual(self.tmpfilename + ".gz.tbi", self.filename_idx)
...
class TestGZFile(IterationTest):
    ...
    def setUp(self):
        super().setUp()
        self.gzfile = fabric.GZIterator(self.filename)
...
class TestIterationWithoutComments(IterationTest):
    ...
    def setUp(self):
        super().setUp()
        self.tabix = fabric.TabixFile(self.filename)
    ...
    def testReopening(self):
        ...
        def func1():
            ...
            with fabric.TabixFile(self.filename) as inf:
                pass
...
class TestIterators(unittest.TestCase):
    ...
    iterator = fabric.tabix_generic_iterator
    parser = fabric.asTuple
    is_compressed = False
...
class TestIteratorsFileCompressed(TestIterators):
    iterator = fabric.tabix_file_iterator
    is_compressed = True
...
class TestBed(unittest.TestCase):
    ...
    def testRead(self):
        ...
        for x, r in enumerate(self.tabix.fetch(parser=fabric.asBed())):
            c = self.compare[x]
...
class TestVCF(unittest.TestCase):
    ...
    def setUp(self):
        ...
        shutil.copyfile(self.filename, self.tmpfilename)
        fabric.tabix_index(self.tmpfilename, preset="vcf")
...
class TestUnicode(unittest.TestCase):
    ...
    def testFromTabix(self):
        ...
        with fabric.TabixFile(
                self.tmpfilename + ".gz", encoding="ascii") as t:
            results = list(t.fetch(parser=fabric.asVCF()))
...
class TestVCFFromTabix(TestVCF):
    ...
    def setUp(self):
        ...
        self.tabix = fabric.TabixFile(self.tmpfilename + ".gz")
        self.compare = load_and_convert(self.filename)
    ...
    def testRead(self):
        ...
        for x, r in enumerate(self.tabix.fetch(parser=fabric.asVCF())):
            c = self.compare[x]
...
class TestVCFFromVariantFile(TestVCFFromVCF):
    ...
    def get_iterator(self):
        self.vcf = fabric.VariantFile(self.filename)
    ...
    def open_vcf(self, fn):
        with fabric.VariantFile(fn) as inf:
            pass
...
class TestRemoteFileHTTP(unittest.TestCase):
    ...
    url = "http://genserv.anat.ox.ac.uk/downloads/fabric/test/example.gtf.gz"
    region = "chr1:1-1000"
    local = os.path.join(TABIX_DATADIR, "example.gtf.gz")

    def setUp(self):
        if not getattr(fabric.config, "HAVE_LIBCURL", 0) or not check_url(self.url):
            self.remote_file = None
        else:
            self.remote_file = fabric.TabixFile(self.url, "r")

        self.local_file = fabric.TabixFile(self.local, "r")
...
class TestRemoteFileHTTPWithHeader(TestRemoteFileHTTP):
    ...
    url = "http://genserv.anat.ox.ac.uk/downloads/fabric/test/example_comments.gtf.gz"
    region = "chr1:1-1000"
...
def _TestMultipleIteratorsHelper(filename, multiple_iterators):
    ...
    tabix = fabric.TabixFile(filename)
    iterator = tabix.fetch(parser=fabric.asGTF(),
...
class TestBackwardsCompatibility(unittest.TestCase):
    ...
    old version is accessed from fabric"""

    def check(self, filename, raises=None):
        with fabric.TabixFile(filename) as tf:
...
class TestMultipleIterators(unittest.TestCase):
    ...
    def testJoinedIterators(self):
        ...
        with fabric.TabixFile(self.filename) as tabix:
            a = next(tabix.fetch(parser=fabric.asGTF()))
            b = next(tabix.fetch(parser=fabric.asGTF()))
    ...
    def testDisjointIterators(self):
        # two iterators working on the same file
        with fabric.TabixFile(self.filename) as tabix:
            a = next(tabix.fetch(parser=fabric.asGTF(), multiple_iterators=True))
            b = next(tabix.fetch(parser=fabric.asGTF(), multiple_iterators=True))
    ...
    def testDoubleFetch(self):
        ...
        with fabric.TabixFile(self.filename) as f:
...
class TestContextManager(unittest.TestCase):
    ...
    def testManager(self):
        ...
        with fabric.TabixFile(self.filename) as tabixfile:
            tabixfile.fetch()
...
class TestMultithreadTabixFile(unittest.TestCase):
    ...
    def testMultithreadEqualsSinglethread(self):
        ...
        with fabric.TabixFile(self.filename, threads=2) as tabixfile:
