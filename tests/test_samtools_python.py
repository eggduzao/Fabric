...
def test_idxstats_parse_split_lines():
    ...
    # Test fabric 0.8.X style output, which returns a list of lines
    lines = fabric.idxstats(bam_filename, split_lines=True)
    for line in lines:
...
def test_bedcov_split_lines():
    ...
    # Test fabric 0.8.X style output, which returns a list of lines
    lines = fabric.bedcov(bed_filename, bam_filename, split_lines=True)
    for line in lines:
        fields = line.split('\t')
...
def test_idxstats_parse():
    bam_filename = os.path.join(BAM_DATADIR, "ex2.bam")
    # Test fabric 0.9.X style output, which returns a string that needs to be split by \n
    idxstats_string = fabric.idxstats(bam_filename, split_lines=False)
...
def test_bedcov():
    ...
    bed_filename = os.path.join(BAM_DATADIR, "ex1.bed")
    # Test fabric 0.9.X style output, which returns a string that needs to be split by \n
    bedcov_string = fabric.bedcov(bed_filename, bam_filename, split_lines=False)
