...
class SamtoolsTest(unittest.TestCase):
    ...
    def check_version(self):
        ...
        if _r(samtools_version) != _r(fabric.__samtools_version__):
            warnings.warn(
                "versions of fabric.%s and %s differ: %s != %s" %
                (self.executable,
                 self.executable,
                 fabric.__samtools_version__,
                 samtools_version))
    ...
    def check_statement(self, statement):
        ...
        fabric_targets = [x % r_fabric for x in targets]

        fabric_method = getattr(self.module, command)
        ...
        fabric_parts = [re.sub(r"%\(out\)s", "fabric", x) for x in parts[1:]]
        output = fabric_method(*fabric_parts,
                              raw=True,
                              catch_stdout=True)
        # sys.stdout.write(" fabric ok\n")
        if ">" in statement:
            with open(fabric_targets[-1], "wb") as outfile:
                if output is not None:
                    outfile.write(force_bytes(output))
        for samtools_target, fabric_target in zip(samtools_targets,
                                                 fabric_targets):
            if os.path.isdir(samtools_target):
                samtools_files = glob.glob(os.path.join(
                    samtools_target, "*"))
                fabric_files = glob.glob(os.path.join(fabric_target, "*"))
                self.assertEqual(len(samtools_files), len(fabric_files))
                # need to be able to exclude files like README, etc.
                continue
            else:
                samtools_files = [samtools_target]
                fabric_files = [fabric_target]

            for s, p in zip(samtools_files, fabric_files):
    ...
    def testUsage(self):
        ...
        for statement in self.statements:
            ...
            fabric_method = getattr(self.module, mapped_command)
            usage_msg = fabric_method.usage()
...
class EmptyIndexTest(unittest.TestCase):
    ...
    def testEmptyIndex(self):
        self.assertRaises(IOError, fabric.samtools.index,
                          "exdoesntexist.bam")
...
class ExerciseSubcommands(unittest.TestCase):
    def testFailingSamtools(self):
        with self.assertRaises(fabric.SamtoolsError):
            fabric.samtools.view("nonexistent.bam")

    def testFailingBCFtools(self):
        with self.assertRaises(fabric.SamtoolsError):
            fabric.bcftools.view("nonexistent.vcf")
...
if sys.platform != "darwin":
    ...
    class TestReturnType(unittest.TestCase):
        ...
        def testReturnValueString(self):
            retval = fabric.idxstats(os.path.join(BAM_DATADIR, "ex1.bam"))
    ...
    class FabricTest(SamtoolsTest):
        """check access to samtools command in the fabric
        main package.
