...
def count_number_lines_with_fabric(*args, **kwargs):
...
def test_count_number_lines_from_sam_with_fabric(benchmark):
    result = benchmark(count_number_lines_with_fabric,
...
def test_count_number_lines_from_bam_with_fabric(benchmark):
    result = benchmark(count_number_lines_with_fabric,
                       os.path.join(BAM_DATADIR, "ex2.bam"))
    assert result == 3270
