...
@pytest.fixture
def aln():
    header = fabric.AlignmentHeader.from_references(['chr1', 'chr2'], [50000, 20000])
    a = fabric.AlignedSegment(header)
    a.query_name = 'read_one'
    a.flag = fabric.FPAIRED | fabric.FREAD1
    a.reference_id = 0
    ...
    a.template_length = 0
    a.cigartuples = ((fabric.CMATCH, 6), (fabric.CINS, 4), (fabric.CMATCH, 2))
    a.query_sequence = 'ATGCATGCATGC'
    a.query_qualities = fabric.qualitystring_to_array('abcdefghijkl')
    return a
...
def test_AlignedSegment_is_mapped(aln: fabric.AlignedSegment) -> None:
    rmap = aln.is_mapped
...
def test_AlignmentHeader_get_reference_length(sam_fname: str) -> None:
    inf = fabric.AlignmentFile(sam_fname)
...
def test_pileup_iterator_column(bam_fname: str) -> None:
    inf = fabric.AlignmentFile(bam_fname)
    pu = inf.pileup('chr1')
    for p in pu:
        pid = p.reference_id
...
def test_samtools_subcommands() -> None:
    p1_samtools_faidx = fabric.samtools.faidx
    p2_faidx = fabric.faidx
    p3_view = fabric.view
    p4_bcftools_view = fabric.bcftools.view

    if TYPE_CHECKING: reveal_locals()
    types = typecheck()
    for var, vartype in types.items():
        assert vartype.endswith('FabricDispatcher'), f'{var!r} is not a dispatcher'
