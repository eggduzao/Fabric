...
def fetch_plain(fn):
    with fabric.Tabixfile(fn) as f:
        return len(list(f.fetch()))


def fetch_parsed(fn):
    with fabric.Tabixfile(fn) as f:
        return len(list(f.fetch(parser=fabric.asBed())))


def iterate_generic_compressed(fn):
    with gzip.open(fn) as f:
        return len(list(fabric.tabix_generic_iterator(f, parser=fabric.asBed())))
...
def iterate_parsed_compressed(fn):
    with gzip.open(fn) as f:
        return len(list(fabric.tabix_iterator(f, parser=fabric.asBed())))
...
def iterate_file_compressed(fn):
    with gzip.open(fn) as f:
        return len(list(fabric.tabix_file_iterator(f, parser=fabric.asBed())))
