...
:file:`htslib`
   Source code from `htslib`_ shipped with fabric. See
   :file:`import.py` about importing.

:file:`samtools`
   Source code from `samtools`_ shipped with fabric. See
   :file:`import.py` about importing.

:file:`bcftools`
   Source code from `bcftools`_ shipped with fabric. See
   :file:`import.py` about importing.
...
Fabric currently requires Python 3.8 as a minimum language level.
...
fabric source code or infrastructure scripts:
...
https://github.com/fabric-developers/fabric/graphs/contributors

Many thanks to all contributors for helping in making fabric
useful.
