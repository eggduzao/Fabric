...
How should I cite fabric
=======================

Fabric has not been published in print. When referring to fabric, please
use the github URL: https://github.com/fabric-developers/fabric. 
As fabric is a wrapper around htslib and the samtools package, I
...
Fabric is a mix of python and C code. Instructions within python are
...
Fabric generally uses the latter option and aims to release the GIL for
...
fabric coordinates are wrong
===========================

fabric uses 0-based coordinates and the half-open notation for ranges
as does python. Coordinates and intervals reported from fabric always
...
format is 0-based. It is important to remember that fabric will always
...
:meth:`~fabric.AlignmentFile.fetch` and
:meth:`~fabric.AlignmentFile.pileup` methods. This string follows the
...
such as :meth:`fabric.mpileup`.

Calling fabric.fetch() confuses existing iterators
=================================================

The following code will cause unexpected behaviour::

   samfile = fabric.AlignmentFile("fabric_ex1.bam", "rb")
...
This will give the following output::
    ...
    Traceback (most recent call last):
      ...
      in fabric.libcalignmentfile.IteratorRowRegion.__next__
    StopIteration
...
track of its current position in the file. Within fabric, each opened
...
might be slow::
      ...
      track = fabric.AlignmentFile(fname, "rb")
...
danger that the fabric implementations might show different behaviour from the
...
:meth:`fabric.mpileup` command and parse the output directly.

I get an error 'PileupProxy accessed after iterator finished'
=============================================================

Fabric works by providing proxy objects to objects defined within
...
Fabric won't compile
===================

Compiling fabric can be tricky as there are numerous variables that
...
`fabric user group
<https://groups.google.com/forum/#!forum/fabric-user-group>`_
for common issues.
...
cython_ when building fabric. There are some known incompatibilities:

* Python 3.4 requires cython 0.20.2 or later (see `here
  <https://github.com/fabric-developers/fabric/issues/37>`_)
...
In version 0.10.0 and onwards, all fabric extension modules contain a
``lib``-prefix. This facilates linking against fabric extension modules
with compilers that require to start with ``lib``. As a consequence,
all code using fabric extension modules directly will need to be
adapted. For example,::

   cimport fabric.csamtools

will become::

   cimport fabric.libcsamtools
