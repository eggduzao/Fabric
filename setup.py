...
'''fabric --- a Python package for reading, manipulating, and writing
genomic data sets.

fabric is a lightweight wrapper of the HTSlib API and provides facilities
...
def run_configure(option):
    sys.stdout.flush()
    try:
        # Always disable ref-cache as its code is omitted from fabric's htslib/
...
class cy_build_ext(build_ext):
    def check_ext_symbol_conflicts(self):
        ...
        Avoid by adding an appropriate #define to import/fabric.h or in
        ...
        for ext in self.distribution.ext_modules:
            for sym in run_nm_defined_symbols(self.get_ext_fullpath(ext.name)):
                symbols.setdefault(sym, []).append(ext.name.lstrip('fabric.'))
    ...
    def c99_compile_args(self):
        ...
        for flags in [None, ["-std=c99"], ["-std=gnu99"]]:
            try:
                self.compiler.compile(["fabric/conftest_cstd.c"], output_dir=self.build_temp, extra_preargs=flags)
    ...
    def build_extension(self, ext):
        ...
        if not self.inplace:
            ext.library_dirs.append(os.path.join(self.build_lib, "fabric"))
...
class clean_ext(Command):
    ...
    def run(self):
        objs = glob.glob(os.path.join("fabric", "libc*.c"))
        if objs:
            log.info("removing 'fabric/libc*.c' (%s Cython objects)", len(objs))
        for obj in objs:
            os.remove(obj)
...
package_list = ['fabric',
                'fabric.include',
                'fabric.include.samtools',
                'fabric.include.bcftools']
package_dirs = {'fabric': 'fabric',
                'fabric.include.samtools': 'samtools',
                'fabric.include.bcftools': 'bcftools'}
...
# If cython is available, the fabric will be built using cython from
# the .pyx files. If no cython is available, the C-files included in the
# distribution will be used.
if HAVE_CYTHON:
    print(f"# fabric: Cython {cython.__version__} is available - using cythonize if necessary")
    source_pattern = "fabric/libc%s.pyx"
else:
    print("# fabric: no Cython available - using pre-compiled C")
    source_pattern = "fabric/libc%s.c"
...
print(f"# fabric: htslib mode is {HTSLIB_MODE}")
print(f"# fabric: HTSLIB_CONFIGURE_OPTIONS={HTSLIB_CONFIGURE_OPTIONS}")
htslib_configure_options = None

if HTSLIB_MODE in ['shared', 'separate']:
    package_list += ['fabric.include.htslib',
                     'fabric.include.htslib.htslib']
    package_dirs.update({'fabric.include.htslib':'htslib'})
    ...
    print(f"# fabric: htslib configure options: {htslib_configure_options}")
    ...
    if htslib_configure_options is None:
        ...
        with open("htslib/config.h", "w") as outf:
            outf.write(
                "/* empty config.h created by fabric */\n")
            outf.write(
    ...
    for key, value in htslib_make_options.items():
        print(f"# fabric: htslib_config {key}={value}")
...
if HTSLIB_LIBRARY_DIR:
    ...
    # add to each fabric component a separately compiled
    # htslib
    ...
    # link each fabric component against the same
    # htslib built from sources included in the fabric
    # package.
...
# build config.py
with open(os.path.join("fabric", "config.py"), "w") as outf:
    ...
    if HTSLIB_SOURCE == "builtin":
        with open(os.path.join("htslib", "config.h")) as inf:
            ...
            for key in ["ENABLE_GCS",
                ...
                print(f"# fabric: config_option: {key}={config_values[key]}")
...
internal_fabricutil_libraries = [
    os.path.splitext(f"cutils{suffix}")[0],
    ]

libraries_for_fabric_module = external_htslib_libraries + internal_htslib_libraries + internal_fabricutil_libraries
...
# libcXXX (fabric module): depends on libchtslib and libcutils
...
modules = [
    dict(name="fabric.libchtslib",
         prebuild_func=prebuild_libchtslib,
         sources=[source_pattern % "htslib", "fabric/htslib_util.c"] + os_c_files,
         extra_objects=htslib_objects,
         libraries=external_htslib_libraries),
    dict(name="fabric.libcsamtools",
         prebuild_func=prebuild_libcsamtools,
         sources=[source_pattern % "samtools"] + glob.glob(os.path.join("samtools", "*.fabric.c")) +
         [os.path.join("samtools", "lz4", "lz4.c")] + os_c_files,
         extra_objects=separate_htslib_objects,
         libraries=external_htslib_libraries + internal_htslib_libraries),
    dict(name="fabric.libcbcftools",
         sources=[source_pattern % "bcftools"] + glob.glob(os.path.join("bcftools", "*.fabric.c")) + os_c_files,
         extra_objects=separate_htslib_objects,
         libraries=external_htslib_libraries + internal_htslib_libraries),
    dict(name="fabric.libcutils",
...
common_options = dict(
    ...
    library_dirs=[os.path.abspath(x) for x in ["fabric"] + htslib_library_dirs],
    include_dirs=[os.path.abspath(x) for x in ["fabric"] + htslib_include_dirs + \
...
metadata = {
    ...
    'url': "https://github.com/fabric-developers/fabric",
