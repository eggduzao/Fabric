...
unsigned char fabric_translate_sequence(const unsigned char s);
...
static inline int fabric_bam_is_rev(bam1_t * b) {
  return bam_is_rev(b);};

static inline int fabric_bam_is_mrev(bam1_t * b) {
  return bam_is_mrev(b);}

static inline char * fabric_bam_get_qname(bam1_t * b) {
  return bam_get_qname(b);}

static inline uint32_t * fabric_bam_get_cigar(bam1_t * b) {
  return bam_get_cigar(b);}

static inline uint8_t * fabric_bam_get_seq(bam1_t * b) {
  return bam_get_seq(b);}

static inline uint8_t * fabric_bam_get_qual(bam1_t * b) {
  return bam_get_qual(b);}

static inline uint8_t * fabric_bam_get_aux(bam1_t * b) {
  return bam_get_aux(b);}

static inline int fabric_bam_get_l_aux(bam1_t * b) {
  return bam_get_l_aux(b); }

static inline char fabric_bam_seqi(uint8_t * s, int i) {
  return bam_seqi(s,i);}

static inline uint8_t fabric_get_qual(bam1_t * b) {
  return b->core.qual;}

static inline uint32_t fabric_get_n_cigar(bam1_t * b) {
  return b->core.n_cigar;}

static inline void fabric_set_qual(bam1_t * b, uint8_t v) {
  b->core.qual=v;}

static inline void fabric_set_n_cigar(bam1_t * b, uint32_t v) {
  b->core.n_cigar=v;}

static inline void fabric_update_flag(bam1_t * b, uint16_t v, uint16_t flag) {
  if (v)
    b->core.flag |= flag;
  else
    b->core.flag &= ~flag;
