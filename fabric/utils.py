...
from fabric.libcutils import _fabric_dispatch
...
class FabricDispatcher(object):
    '''The dispatcher emulates the samtools/bctools command line.

    Captures stdout and stderr.

    Raises a :class:`fabric.SamtoolsError` exception in case samtools
    ...
    def __call__(self, *args: str, **kwargs) -> Union[str, List[str]]:
        ...
        '''
        retval, stderr, stdout = _fabric_dispatch(
            self.collection,
            self.dispatch,
            args,
...
class unquoted_str(str):
    ...
    header takes the form of key=value pairs. By default, fabric will
