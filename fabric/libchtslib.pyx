...
from fabric.libcutils cimport encode_filename, from_string_and_size, libc_whence_from_io
...
cdef class HTSFile(object):
    ...
    def seek(self, uint64_t offset, int whence=io.SEEK_SET):
        """move file pointer to position *offset*, see :meth:`fabric.HTSFile.tell`."""
        if not self.is_open:
    ...
    def tell(self):
        """return current file position, see :meth:`fabric.HTSFile.seek`."""
        if not self.is_open:
            raise ValueError('I/O operation on closed file')
        if self.is_stream:
