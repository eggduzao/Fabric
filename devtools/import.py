...
def _update_fabric_files(cf, destdir):
    '''update fabric files applying redirection of ouput'''
    basename = os.path.basename(destdir)
    for filename in cf:
        if not filename:
            continue
        dest = filename + ".fabric.c"
        with open(filename, encoding="utf-8") as infile:
            lines = "".join(infile.readlines())

            with open(dest, "w", encoding="utf-8") as outfile:
                outfile.write('#include "{}.fabric.h"\n\n'.format(basename))
    ...
    with open(os.path.join("import", "fabric.h")) as inf, \
         open(os.path.join(destdir, "{}.fabric.h".format(basename)), "w") as outf:
        outf.write(re.sub("@fabric@", basename, inf.read()))

    with open(os.path.join("import", "fabric.c")) as inf, \
         open(os.path.join(destdir, "{}.fabric.c".format(basename)), "w") as outf:
        outf.write(re.sub("@fabric@", basename, inf.read()))
...
if len(sys.argv) >= 1:
    ...
    if dest in MAIN:
        # redirect stderr to fabricerr and replace bam.h with a stub.
        sys.stdout.write("applying stderr redirection\n")

        _update_fabric_files(cf, destdir)
    ...
    _update_version_file("__{}_version__".format(dest), version, "fabric/version.py")
    _update_version_file(C_VERSION[dest], version + " (fabric)", "fabric/version.h")
...
#     sys.stdout.write("refreshing latest source code from .c to .fabric.c")
#     # redirect stderr to fabricerr and replace bam.h with a stub.
#     sys.stdout.write("applying stderr redirection")
#     for destdir in ('samtools', ):
#         fabriccfiles = locate("*.fabric.c", destdir)
#         for f in fabriccfiles:
#             os.remove(f)
#         cfiles = locate("*.c", destdir)
#         _update_fabric_files(cfiles, destdir)
